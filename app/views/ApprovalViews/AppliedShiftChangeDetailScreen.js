...
const AppliedShiftchangeDetailScreen = ({navigation}) => {
    ...
    const [shiftChangeData, setShiftChangeData] = useState({
        'applied': [],
        'pending': [],
        'approved': [],
        'rejected': []
    ...
    const handleAction = async (shiftChangeRequestId, remarks, isApproved, isRecommended, recommendReject, approveReject) => {
        const body =  {
    ...
    return (
        <View style={styles.container}>
            <View style={styles.fixedCard}>
                <ShiftChangeStatsCard stats={counts} onNumberPress={handleNumberPress}/>
            </View>
            ...
            >
                <View>
                    {shiftChangeData[menuTab] && shiftChangeData[menuTab].map((data, index) => {
                        let status = '';
                        ...
                        // isRejected = false;
                            ...
                            return (<RequestCard2
                                ...
                                fields={fields}
                                onApprove={() => handleAction(data.shiftChangeRequestId, data.remarks, true, data.isRecommended, data.recommendReject, data.approveReject)}
                                onReject={() => handleAction(data.shiftChangeRequestId, data.remarks, data.isApproved, data.isRecommended, data.recommendReject, true)}
                                onRecommend={() => handleAction(data.shiftChangeRequestId, data.remarks, data.isApproved, true, data.recommendReject, data.approveReject)}
                                onRecommendReject={() => handleAction(data.shiftChangeRequestId, data.remarks, data.isApproved, data.isRecommended, true, data.approveReject)}
...
export default AppliedShiftchangeDetailScreen;

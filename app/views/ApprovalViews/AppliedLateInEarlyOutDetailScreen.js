...
const LateInEarlyOutStatsCard = ({stats, onNumberPress}) => {
...
const AppliedLateInEarlyOutDetailScreen = ({navigation}) => {
...
const [isAD, setIsAD] = useState(false);
    ...
    const fetchData = async () => {
      ...
      clientDetail.useBS ? setIsAD(false) : setIsAD(true);
            ...
            const response = await APIKit.get(`/LateInEarlyOut/GetFilteredLateInEarlyOut/2024-07-16/2025-07-16/true`);
    ...
    const handleAction = async (lateInEarlyOutId, remarks, isApproved, isRecommended, recommendReject, approveReject) => {
        const body =  {
            lateInEarlyOutId,
            remarks,
            isApproved,
        ...
        setIsLoading(true);
        try {
            const response = await APIKit.post('/lateinEarlyout/LateInEarlyOutAction', body);

            Toast.show({
                type: 'success',
                text1: 'Success',
                text2: `Late In Early Out ${action}`
            });
    ...
    return (
        <View style={styles.container}>
            <View style={styles.fixedCard}>
                <LateInEarlyOutStatsCard stats={counts} onNumberPress={handleNumberPress}/>
            </View>
            ...
            >
                <View>
                    {inOutData[menuTab] && inOutData[menuTab].map((data, index) => {
                        let status = '';
                        ...
                        // canApproveAndReject = true;
                            ...
                            return (<RequestCard2
                                ...
                                fields={fields}
                                onApprove={() => handleAction(data.lateInEarlyOutId, data.remarks, true, data.isRecommended, data.recommendReject, data.approveReject)}
                                onReject={() => handleAction(data.lateInEarlyOutId, data.remarks, data.isApproved, data.isRecommended, data.recommendReject, true)}
                                onRecommend={() => handleAction(data.lateInEarlyOutId, data.remarks, data.isApproved, true, data.recommendReject, data.approveReject)}
                                onRecommendReject={() => handleAction(data.lateInEarlyOutId, data.remarks, data.isApproved, data.isRecommended, true, data.approveReject)}
                ...
                {/* {menuTab === 'All' && <Text>All</Text>}
                {menuTab === 'Pending' && <Text>Pending</Text>}
                {menuTab === 'Applied' && <Text>Applied</Text>}
                {menuTab === 'Rejected' && <Text>Rejected</Text>}
                {menuTab === 'Approved' && <Text>Approved</Text>} */}
...
export default AppliedLateInEarlyOutDetailScreen;

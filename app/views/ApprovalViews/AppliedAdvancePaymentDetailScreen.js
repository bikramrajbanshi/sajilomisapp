...
const AdvancePaymentStatsCard = ({stats, onNumberPress}) => {
...
const AppliedAdvancePaymentDetailScreen = ({navigation}) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await APIKit.get(`/AdvancePaymentRequest/GetUserFilteredAdvanceRequestList/2024-07-16/2025-07-16/true`);
    ...
    const loadMoreData = (category) => {
        ...
        setAdvancePaymentData(prevData => ({
            ...prevData,
            [category]: [
                ...prevData[category],
    ...
    const handleAction = async (advancePaymentRequestId, remarks, isApproved, isRecommended, recommendReject, approveReject) => {
        const body =  {
        ...
        setIsLoading(true);
        try {
            const response = await APIKit.post('/AdvancePaymentRequest/AdvanceRequestAction', body);

            Toast.show({
                type: 'success',
                text1: 'Success',
                text2: `Advance payment ${action}`
            });
    ...
    return (
        ...
        <AdvancePaymentStatsCard stats={counts} onNumberPress={handleNumberPress}/>
        </View>
        ...
        >
        <View>
        {advancePaymentData[menuTab] && advancePaymentData[menuTab].map((data, index) => {
            let status = '';
            ...
            const fields = [
                ...
                {
                    title: 'Advance Date',
                    value: geFullDate(data.advanceDate, !isAD)
                },
                {
                    title: 'Amount',
                    value: data.amount
            ...
            return (<RequestCard2
                ...
                fields={fields}
                onApprove={() => handleAction(data.advancePaymentRequestId, data.remarks, true, data.isRecommended, data.recommendReject, data.approveReject)}
                onReject={() => handleAction(data.advancePaymentRequestId, data.remarks, data.isApproved, data.isRecommended, data.recommendReject, true)}
                onRecommend={() => handleAction(data.advancePaymentRequestId, data.remarks, data.isApproved, true, data.recommendReject, data.approveReject)}
                onRecommendReject={() => handleAction(data.advancePaymentRequestId, data.remarks, data.isApproved, data.isRecommended, true, data.approveReject)}
                                                                                                 ...
                                                                                                 statsNumber: {
                                                                                                     ...
                                                                                                     statsLabel: {
                                                                                                         ...
                                                                                                         requestContainer: {
                                                                                                            ...
                                                                                                            export default AppliedAdvancePaymentDetailScreen;

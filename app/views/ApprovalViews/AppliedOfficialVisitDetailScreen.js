...
const OfficalVisitStatsCard = ({stats, onNumberPress}) => {
...
const AppliedOfficalVisitDetailScreen = ({navigation}) => {
...
const [isAD, setIsAD] = useState(false);
    ...
    const [officialVisitData, setOfficialVisitData] = useState({
        'applied': [],
        'pending': [],
        'approved': [],
        'rejected': []
    ...
    const fetchData = async () => {
      ...
      clientDetail.useBS ? setIsAD(false) : setIsAD(true);
            ...
            const response = await APIKit.get(`/OfficialVisit/GetUserFilteredOfficialVisitList/2024-07-16/2025-07-16/true`);
    ...
    const handleAction = async (officialVisitId, remarks, isApproved, isRecommended, recommendReject, approveReject) => {
        const body =  {
            officialVisitId,
            remarks,
            isApproved,
        ...
        setIsLoading(true);
        try {
            const response = await APIKit.post('/OfficialVisit/ActionOfficialVisit', body);

            Toast.show({
                type: 'success',
                text1: 'Success',
                text2: `Official visit ${action}`
            });
    ...
    return (
        <View style={styles.container}>
            <View style={styles.fixedCard}>
                <OfficalVisitStatsCard stats={counts} onNumberPress={handleNumberPress}/>
            </View>
            ...
            >
                <View>
                    {officialVisitData[menuTab] && officialVisitData[menuTab].map((data, index) => {
                        let status = '';
                        ...
                        recommender += data.r_LastName ? data.r_LastName : '';
                            ...
                            const fields = [
                                ...
                                {
                                    title: 'Reason',
                                    value: data.officialVisitReason !== null && data.officialVisitReason !== '' ? `${data.officialVisitReason}` : '-'
                                },
                            ...
                            const isRejected = !!(data.recommendReject || data.approveReject);
                            ...
                            return (<RequestCard2
                                ...
                                fields={fields}
                                onApprove={() => handleAction(data.officialVisitId, data.remarks, true, data.isRecommended, data.recommendReject, data.approveReject)}
                                onReject={() => handleAction(data.officialVisitId, data.remarks, data.isApproved, data.isRecommended, data.recommendReject, true)}
                                onRecommend={() => handleAction(data.officialVisitId, data.remarks, data.isApproved, true, data.recommendReject, data.approveReject)}
                                onRecommendReject={() => handleAction(data.officialVisitId, data.remarks, data.isApproved, data.isRecommended, true, data.approveReject)}
...
export default AppliedOfficalVisitDetailScreen;

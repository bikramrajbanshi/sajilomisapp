...
const AppliedLeaveEncashmentDetailScreen = ({navigation}) => {
    ...
    const [leaveEncashmentData, setLeaveEncashmentData] = useState({
        'applied': [],
        'pending': [],
        'approved': [],
        'rejected': []
    ...
    const loadMoreData = (category) => {
        ...
        setLeaveEncashmentData(prevData => ({
            ...prevData,
            [category]: [
                ...prevData[category],
    ...
    const handleAction = async (leaveEncashmentId, remarks, isApproved, isRecommended, recommendReject, approveReject) => {
        const body =  {
            leaveEncashmentId,
            remarks,
            isApproved,
        ...
        setIsLoading(true);
        try {
            const response = await APIKit.post('/leaveEncashment/ActionLeaveEncashment', body);

            Toast.show({
                type: 'success',
                text1: 'Success',
                text2: `Leave encashment ${action}`
            });
    ...
    return (
        <View style={styles.container}>
            <View style={styles.fixedCard}>
                <LeaveEncashmentStatsCard stats={counts} onNumberPress={handleNumberPress}/>
            </View>
            ...
            >
                <View>
                    {leaveEncashmentData[menuTab] && leaveEncashmentData[menuTab].map((data, index) => {
                        let status = '';
                        ...
                        // isRejected = false;
                            ...
                            return (<RequestCard2
                                ...
                                fields={fields}
                                onApprove={() => handleAction(data.leaveEncashmentId, data.remarks, true, data.isRecommended, data.recommendReject, data.approveReject)}
                                onReject={() => handleAction(data.leaveEncashmentId, data.remarks, data.isApproved, data.isRecommended, data.recommendReject, true)}
                                onRecommend={() => handleAction(data.leaveEncashmentId, data.remarks, data.isApproved, true, data.recommendReject, data.approveReject)}
                                onRecommendReject={() => handleAction(data.leaveEncashmentId, data.remarks, data.isApproved, data.isRecommended, true, data.approveReject)}
...
export default AppliedLeaveEncashmentDetailScreen;

...
const AppliedOvertimeDetailScreen = ({navigation}) => {
...
const [isAD, setIsAD] = useState(false);
    ...
    const fetchData = async () => {
      ...
      clientDetail.useBS ? setIsAD(false) : setIsAD(true);
            ...
            const shrawan1stInAD = getShrawan1stInAD(date.getFullYear()-1);
    ...
    const handleAction = async (overTimeApplicationId, remarks, isApproved, isRecommended, recommendReject, approveReject) => {
        const body =  {
        ...
        setIsLoading(true);
        try {
            const response = await APIKit.post('/OverTime/OverTimeAction', body);

            Toast.show({
                type: 'success',
                text1: 'Success',
                text2: `Overtime ${action}`
            });
    ...
    return (
        <View style={styles.container}>
            <View style={styles.fixedCard}>
                <OvertimeStatsCard stats={counts} onNumberPress={handleNumberPress}/>
            </View>
            ...
            >
                <View>
                    {overtimeData[menuTab] && overtimeData[menuTab].map((data, index) => {
                        let status = '';
                        ...
                        // isApproved = false;
                            ...
                            return (<RequestCard2
                                ...
                                fields={fields}
                                onApprove={() => handleAction(data.overTimeApplicationId, data.remarks, true, data.isRecommended, data.recommendReject, data.approveReject)}
                                onReject={() => handleAction(data.overTimeApplicationId, data.remarks, data.isApproved, data.isRecommended, data.recommendReject, true)}
                                onRecommend={() => handleAction(data.overTimeApplicationId, data.remarks, data.isApproved, true, data.recommendReject, data.approveReject)}
                                onRecommendReject={() => handleAction(data.overTimeApplicationId, data.remarks, data.isApproved, data.isRecommended, true, data.approveReject)}
...
export default AppliedOvertimeDetailScreen;

...
const WeekendListScreen = () => {
  ...
  useEffect(() => {
    loadToken();
    fetchWeekendListData();
  }, []);

  const fetchWeekendListData = async () => {
    try {
      ...
      const weekendListData = response.data;
      // console.log(weekendListData);
      const filteredWeekendListData = weekendListData.filter(
          (item) => item.weekend == 'Yes'
      );
      // console.log(filteredWeekendListData);
      setWeekendList(filteredWeekendListData);
    } catch (error) {
      console.error("Error fetching weekend list data:", error);
    } finally {
      setIsLoading(false);
      setRefreshing(false);
    }
...
export default WeekendListScreen;

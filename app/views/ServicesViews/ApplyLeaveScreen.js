...
import { getLeaveTypesWithFullDetails} from '../../utils/apiUtils'; 
...
const ApplyLeaveScreen = ({navigation}) => {
    ...
    const [getLeaveTypesWithDetails, setLeaveTypesWithDetails] = useState([]);
    const [showFromPicker, setShowFromPicker] = useState(false);
    const [showToPicker, setShowToPicker] = useState(false);
    const [canApply, setCanApply] = useState(null);
    const [canApplyHalfDay, setCanApplyHalfDay] = useState(null);
    ...
    useEffect(() => {
        ...
        const loadLeaveType = async () => {
            try {
              const data = await getLeaveTypesWithFullDetails();
              setLeaveTypesWithDetails(data);
          } catch (error) {
    ...
    const getEntitledLeaveCount = async (itemValue) => {
        ...
        try {
            let checkUnPaidLeave;
            if(getLeaveTypesWithDetails.length == 0)
            {
                console.log("abc");
                const leaveTypeatFirst = await getLeaveTypesWithFullDetails();
                checkUnPaidLeave = leaveTypeatFirst.find(leave => leave.leaveId === parseInt(itemValue));
            }
            else
            {
               checkUnPaidLeave = getLeaveTypesWithDetails.find(leave => leave.leaveId === parseInt(itemValue));

           }
           setCanApplyHalfDay(false);
           if(checkUnPaidLeave.canApplyHalfLeave === true)
           {
            console.log("half leave");
            setCanApplyHalfDay(true);
           }
            if(checkUnPaidLeave.isPaid === false)
            {
                setBalanceLeave(0);
                setCanApply(true);
                return;
...
const handleSubmit = async () => {
    ...
    if (canApply == false) {
        Toast.show({
            type: 'error',
            text1: 'Error',
            text2: 'Not Enough Balance For the Selected Leave'
        });
        return;
    }

    try {
        if ((balanceLeave < totalDays || balanceLeave == 0)&& canApply !== true) {
            Toast.show({
                type: 'error',
                text1: 'Error',
...
return (
    ...
    {leaveType != '' ? (
        ...
        <Text style={styles.number}>{balanceLeave}  {canApply == true ? "" : "- Not Enough Balance" }</Text>
        </View>
        </View>
        ) : ('')}

...
const ShiftChangeScreen = ({navigation, route}) => {
    ...
    const fetchShiftChange = async (userId) => {
        ...
        // console.log(`/ShiftChangeRequest/GetUserFilteredShiftChangeList/${formatedStartDate}/${formatedEndDate}/true?id=${userId}`);

        try {
            const response = await APIKit.get(
                `/ShiftChangeRequest/GetUserFilteredShiftChangeList/${formatedStartDate}/${formatedEndDate}/true?id=${userId}`
            );
    ...
    const handleButtonClick = () => {
        fetchShiftChange(selectedUser || userId,).finally(() => {
            setRefreshing(false);
            setFirstLoad(false);
        });
    ...
    return (
        ...
        <Button style={{flex: 1, height: 50, marginHorizontal: 5}} title="Search" onPress={handleButtonClick} />
            ...
            <TouchableOpacity
                style={styles.fab}
                onPress={() => navigation.navigate("ApplyShiftChange")}
            >
...
export default ShiftChangeScreen;
